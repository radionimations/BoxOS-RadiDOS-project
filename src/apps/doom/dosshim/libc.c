...
/* HANG instead of reboot so we can read the screen.
 * Kernel-side: a halt loop with cli; the user must reset the VM. */
void exit(int code)  {
    (void)code;
    bos_puts("\n[DOOM] exit() called -- hanging\n");
    for (;;) { __asm__ __volatile__("cli; hlt"); }
}
void abort(void)     {
    bos_puts("\n[DOOM] abort() called -- hanging\n");
    for (;;) { __asm__ __volatile__("cli; hlt"); }
}
    ...
    for (; *fmt; fmt++) {
        ...
        switch (*fmt) {
            case 'd': case 'i': {
                ...
                emit_int(v, 10, 1, width, zeropad, 0, &out, &rem);
                break;
            }
            case 'u': {
                unsigned long long v = longflag ? va_arg(ap, unsigned long long)
                                                 : va_arg(ap, unsigned int);
                emit_int((long long)v, 10, 0, width, zeropad, 0, &out, &rem);
                break;
            }
            case 'x':
            case 'X': {
                unsigned long long v = longflag ? va_arg(ap, unsigned long long)
                                                 : va_arg(ap, unsigned int);
                emit_int((long long)v, 16, 0, width, zeropad, *fmt == 'X', &out, &rem);
                break;
            }
            case 'p': {

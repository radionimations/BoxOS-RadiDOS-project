...
{
    char name[8];
    boolean masked;          /* 4 bytes (enum=int) on both 32 and 64 bit */
    short width;
    short height;
    /* OBSOLETE field. On disk this is exactly 4 bytes (was a 16-bit
     * DOS far pointer). Must be int32, not void**, otherwise on
     * x86_64 the struct grows by 4 bytes and every field after this
     * (patchcount + patches) is read from the wrong offset. */
    int   columndirectory;
    short patchcount;
...
void R_InitTextures(void)
{
    ...
    /* BoxOS port: original used *4 assuming 4-byte DOS pointers.
     * On x86_64, void* is 8 bytes — use sizeof for the pointer arrays. */
    textures           = Z_MallocUnowned(numtextures * sizeof(*textures),           PU_STATIC);
    texturecolumnlump  = Z_MallocUnowned(numtextures * sizeof(*texturecolumnlump),  PU_STATIC);
    texturecolumnofs   = Z_MallocUnowned(numtextures * sizeof(*texturecolumnofs),   PU_STATIC);
    texturecomposite   = Z_MallocUnowned(numtextures * sizeof(*texturecomposite),   PU_STATIC);
    texturecompositesize = Z_MallocUnowned(numtextures * sizeof(*texturecompositesize), PU_STATIC);
    texturewidthmask   = Z_MallocUnowned(numtextures * sizeof(*texturewidthmask),   PU_STATIC);
    textureheight      = Z_MallocUnowned(numtextures * sizeof(*textureheight),      PU_STATIC);

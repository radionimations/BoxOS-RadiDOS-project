...
/* BoxOS port: keep the global tic counters in sync with wall-clock
 * time. The original DOS port advanced these in a 35Hz/560Hz IRQ
 * handler; we don't have that yet, so poll-update on every entry. */
extern unsigned long ticks_ms(void);
/* Non-static so other translation units (d_main.c spin loops) can
 * pump the wall clock when they're starving NetUpdate. */
void __BoxOS_AdvanceTicCount(void)
{
	unsigned long ms = ticks_ms();
	extern unsigned int ticcount;
	extern unsigned int ticcount_hr;
	ticcount    = (unsigned int)((ms * 35UL) / 1000UL);

...
static void SetVariable(default_t *def, char *value)
{
    ...
    switch (def->type)
    {
        ...
        case DEFAULT_FLOAT:
            /* BoxOS port: SSE/x87 disabled in this build, so float
             * config vars are unsupported. Skip silently. */
            break;
    }
}
...
/* BoxOS port: SSE/x87 disabled — float APIs unsupported, return 0 as int. */
